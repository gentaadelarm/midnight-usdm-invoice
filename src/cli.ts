...
const compiledContract = CompiledContract.make('private-invoice', PrivateInvoice.Contract).pipe(
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   midnight-usdm-private-invoice CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
...
console.log(`  USDM:    ${formatUsdm(usdmBalance)} USDM\n`);
    ...
    try {
      ...
      if (debugState) {
        ...
        console.log('  data instanceof StateValue:', debugState.data?.constructor?.name === 'StateValue');
        console.log('  data instanceof ChargedState:', debugState.data?.constructor?.name === 'ChargedState');
        console.log('  data type:', typeof debugState.data);
        console.log('  operation(createInvoice):', debugState.operation('createInvoice') ? 'YES' : 'NO');
        console.log('  operation(payInvoice):', debugState.operation('payInvoice') ? 'YES' : 'NO');
      }
    ...
    while (running) {
      ...
      switch (choice.trim()) {
        ...
        case '2': {
          console.log('\n  Paying invoice (this may take 30-60 seconds)...');
        ...
        case '3': {
          ...
          try {
            ...
            if (contractState) {
              ...
              console.log('\n  ─── Invoice State ─────────────────────────');
              console.log(
                `  Invoice Amount: ${ledgerState.invoiceAmount.toLocaleString()}`,
              );
              console.log(
                `  Invoice Paid:   ${ledgerState.invoicePaid ? 'YES ✅' : 'NO ❌'}`,
              );
              console.log('');
            } else {
